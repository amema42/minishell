...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/03/15 15:24:24 by amema             #+#    #+#             */
/*   Updated: 2024/03/15 15:24:25 by amema            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	path_extr(t_general *general)
{
	int	index;
...
int	build_matrix(char *str, t_lex *node, t_general *general)
{
	...
	while (i < matrixlen(node->command2))
	{
		if (handle_quotes(node->command2[i]) == 1)
			return (1);
		quotes_main_manager(node->command2[i], general, i);
		i++;
	}
